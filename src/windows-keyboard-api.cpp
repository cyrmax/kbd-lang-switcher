...
std::optional<std::vector<KeyboardLayoutInfo>> KeyboardManager::getKeyboardLayouts() const {
    ...
    for (size_t i = 0; i < 10 && i < result; ++i) {
        auto langName = getLanguageName(layoutsBuffer[i]);
        auto layoutName = getKeyboardLayoutName(layoutsBuffer[i]);
        if (langName.has_value() && layoutName.has_value()) {
            layouts.emplace_back(KeyboardLayoutInfo(langName.value(), layoutName.value(), layoutsBuffer[i]));
        }
    }
    return layouts;
}
